...
from fairseq.aggregator_log_backend import AggregatorBackend
...
def score(args, trainer, dataset, src_dict, tgt_dict, ref_file):
	...
	[model], len(tgt_dict),
        pad_id=tgt_dict.pad(), unk_id=tgt_dict.unk(), eos_id=tgt_dict.eos(),
    ...
    sacrebleu_score = sacrebleu.corpus_bleu(predictions, refs, lowercase=args.ignore_case).score
    ...
    print('| Eval completed in: {:.2f}s | BLEU {:.2f}'.format(time.time()-begin, sacrebleu_score))
...
def setup_logger(args):
    aggregator_dict = collections.OrderedDict([
        ('loss', 'average'),
    ...
    if not args.distributed_world_size > 1 or args.distributed_rank == 0:
        DLLogger.init(backends = [JSONStreamBackend(verbosity=1, filename=log_path), AggregatorBackend(verbosity=0, agg_dict=aggregator_dict)])
    else:
