...
- [Performance](#performance)
    ...
    * [Results](#results)
        ...
        * [Training performance results](#training-performance-results)
            * [NVIDIA DGX-1 (8x V100 16G)](#nvidia-dgx-1-(8x-v100-16G))
            * [NVIDIA DGX-1 32G (8x V100 32G)](#nvidia-dgx-1-32g-(8x-v100-32G))
...
identically in every process spawned by torch.distributed.launch. Distributed strategy is implemented with APEX's DistributedDataParallel.
...
APEX - The purpose of the APEX is to provide easy and intuitive framework for distributed training and mixed precision training.
...
To benchmark the training performance on a specific batch size, just run `train.py` training script. Performance in words/s will be printed to standard output every N iterations, specified by the `--log-interval` option. Additionally performance and loss values will be logged by [dllogger](https://github.com/NVIDIA/dllogger) to the file specified in `--stat-file` option. Every line in the output file will be a valid JSON file prepended with `DLLL` prefix.

#### Inference performance benchmark

Due to large preprocessing overhead `inference.py` script cannot be used for benchmarking performance. During training the inference performance is measured during the validation stage.
...
To take into account the varibaility of the results we computed basic statistics that help us verify whether a model trains correctly. Evaluating nearly 2000 checkpoints from 20 runs, the best score we achieved is 28.02 BLEU (which corresponds to 28.7 old score). Variance of the score of the best performing model between those 20 runs is 0.011. Knowing that max statistic is skewed toward higher values we have also run studies which calculate threshold beyond which validation loss is no longer correlated with BLEU score.
...
Below we present a plot of our best run. Using batch size 5120 per GPU, learning rate 6e-4, mixed precision trainig and the pytorch-19.09-py3 Docker container.
<p align="center">
    <img src="./transformer_plot.png" />
    <br>
    Figure 4. Training stats for the best run.
</p>


##### NVIDIA DGX-1 (8x V100 16G)

Our results were obtained by running the `run_training.sh` training script in the PyTorch NGC container on NVIDIA DGX-1 with (8x V100 16G) GPUs. Performance numbers (in tokens per second) were averaged over an entire training epoch. Mixed precision training has lower memory requirements, so we can train with batch size twice as big. Batch size for FP32 is 2560 tokens, and for AMP is 5120 tokens.
...
|--------|----------------------|---------------------------------|-----------------------------------------------|------------------------|------------------------------
|8       | 57086                | 236043                          | 4.13                                          |  6.70                  | 6.73 
|4       | 29179                | 121768                          | 4.17                                          |  3.45                  | 3.44
|1       | 8485                 | 35255                           | 4.15                                          |  1                     | 1

To achieve these same results, follow the steps in the [Quick Start Guide](#quick-start-guide).

##### NVIDIA DGX-1 32G (8x V100 32G)

Our results were obtained by running the `run_training.sh` training script in the Pytorch NGC container on NVIDIA DGX-2 with (16x V100 32G) GPUs. Performance numbers (in tokens per second) were averaged over an entire training epoch. 32G cards allowed us to use batch size of 10240 for mixed precision training and 5120 for FP32 training. 

| GPUs   | Throughput - FP32    | Throughput - mixed precision    | Throughput speedup (FP32 - mixed precision)   | Weak scaling - FP32    | Weak scaling - mixed precision        
|--------|----------------------|---------------------------------|-----------------------------------------------|------------------------|-----------------------------
| 8      | 63261                | 261245                          | 4.13                                          | 7.00                   | 6.90                        
| 4      | 31918                | 124088                          | 4.20                                          | 3.53                   | 3.54                        
| 1      | 9041                 | 37876                           | 4.19                                          | 1                      | 1                         
...
| GPUs   | Throughput - FP32    | Throughput - mixed precision    | Throughput speedup (FP32 - mixed precision)   | Weak scaling - FP32    | Weak scaling - mixed precision        
|--------|----------------------|---------------------------------|-----------------------------------------------|------------------------|-----------------------------
| 16     | 130656               | 544012                          | 4.16                                          | 13.46                  | 13.27                        
| 8      | 68043                | 282683                          | 4.15                                          | 7.01                   | 6.89                        
| 4      | 34552                | 145847                          | 4.22                                          | 3.56                   | 3.56                        
| 1      | 9706                 | 41002                           | 4.22                                          | 1                      | 1                         
 
To achieve these same results, follow the steps in the [Quick Start Guide](#quick-start-guide).

#### Inference performance results

We have benchmarked the inference performance by running the `train.py` script using the pytorch-20.03-py3 NGC Docker container. This approach was chosen to remove preprocessing overhead. Inference was run on a single GPU.

GPU                  | Mixed precision | FP32 | FP16/Mixed speedup
---                  |---              |---   |---
Tesla V100-SXM2-32GB | 5424            | 1595 | 1.76
